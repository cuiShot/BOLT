...
from mycaafe import data  # 项目自定义模块
project_root = '/home/usr01/cuicui/autoML-ensemble'
...
def load_origin_data(dataset_name, seed=0):
    ...
    if is_old:
        ...
        if 'credit' in name_l:
            ds, df_train, df_test = data.get_data_split(ds, seed=seed)
        else:
    ...
    # 否则读取新的 CSV 数据集
    base_loc = "/home/usr01/cuicui/autoML-ensemble/new_dataSet/binaryclass/"
...
def load_new_dataset(dataset_name: str,
                     base_loc: str = "/home/usr01/cuicui/autoML-ensemble/new_dataSet/binaryclass/",
                     seed: int = 42,
                     test_size: float = 0.2):
    """
